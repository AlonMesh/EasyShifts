...
class TestWorkPlacesRepository(TestCase):
    ...
    def test_get_workplace_by_worker_id(self):
        workplace_data = {'username': 'Test_Place', 'password': 'pass', 'isManager': 1, 'isActive': 1, 'name': 'Place'}
        worker_data = {'username': 'Test_Worker', 'password': 'pass', 'isManager': 0, 'isActive': 1, 'name': 'Worker'}

        # Initialize the users controller, passing the database session
        users_controller = UsersController(self.db)

        # Create a workplace and worker, connect them via a workplace entity
        workplace = users_controller.create_entity(workplace_data)
        worker = users_controller.create_entity(worker_data)
        workplace_entity = self.controller.create_entity({'id': worker.id, 'workPlaceID': workplace.id})

        try:
            # Check if the workplace & worker exist
            self.assertIsNotNone(workplace)
            self.assertIsNotNone(worker)
            self.assertIsNotNone(workplace_entity)

            print("workplace.id: ", workplace.id)
            print("worker.id: ", worker.id)
            print("workplace_entity.id: ", workplace_entity.id)
            print("4th: ", self.controller.get_entity(worker.id))

            # Check simple get_entity
            self.assertEqual(workplace.id, self.controller.get_entity(worker.id).workPlaceID)

            # Get the workplace by the worker id
            workplace = self.controller.get_workplace_by_worker_id(worker.id)

            # Assert that the workplace is as expected
            self.assertEqual(workplace, workplace_entity)
        finally:
            # Delete the workplace and worker
            user_id = workplace.id
            workplace_id = workplace.workPlaceID
            self.controller.delete_entity(user_id)  # Delete the workplace entity
            users_controller.delete_entity(user_id)  # Delete the worker
            users_controller.delete_entity(workplace_id)  # Delete the manager

    def test_get_workplace_z_and_workplace_name_by_worker_id(self):
        workplace_data = {'username': 'Test_Place', 'password': 'pass', 'isManager': 1, 'isActive': 1, 'name': 'Place'}
        worker_data = {'username': 'Test_Worker', 'password': 'pass', 'isManager': 0, 'isActive': 1, 'name': 'Worker'}
        ...
        try:
            ...
            self.assertEqual("Place", self.controller.get_workplace_name_by_worker_id(worker.id))
